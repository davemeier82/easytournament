      ...
      if (filename != null) {
        try {
          if (chooser.getFileFilter().equals(htmlfilter)) {
            ...
            TablesHTMLHandler.saveTables(filename, Organizer.getInstance().getCurrentTournament().getPlan().getOrderedGroups());
          }
          else {
            if (!filename.getPath().toLowerCase().endsWith(".csv")) {
              filename = new File(filename.getPath() + ".csv");
            }
            TablesCSVHandler.saveTables(filename, Organizer.getInstance().getCurrentTournament().getPlan().getOrderedGroups());
          }
        }
