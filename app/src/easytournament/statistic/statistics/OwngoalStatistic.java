...
public class OwngoalStatistic implements Statistic {

  protected Tournament t = Organizer.getInstance().getCurrentTournament();

  @Override
  public String getName() {
    return ResourceManager.getText(Text.OWNGOALS);
  }
  ...
  public void addStatistic(StatisticTableModel tableModel, boolean groupByTeam,
    ...
    if (groupByTeam) {
      ...
      strCols.clear();
      strCols.add(0);
      tableModel.setColumnIdentifiers(new String[] {ResourceManager.getText(Text.TEAM), ResourceManager.getText(Text.OWNGOALS)});
      ...
      for (ScheduleEntry se : t.getSchedules()) {
        if (se.isGamePlayed())
          for (GameEventEntry ge : se.getGameEvents()) {
            if (ge.getMainPlayers().size() > 0) {
              if (!ge.getTeam().getPlayers()
                  .contains(ge.getMainPlayers().get(0))) {
                Team tm = ge.getTeam().equals(se.getHomePos().getTeam())? se.getAwayTeam() : se.getHomeTeam();
                entryMap.get(tm.getId())[1] += ge.getEvent().getPointsForTeam();
              }
            }
          }
      }
      ...
      tableModel.setColumnIdentifiers(new String[] {ResourceManager.getText(Text.PLAYER), ResourceManager.getText(Text.TEAM),
          ResourceManager.getText(Text.OWNGOALS)});
